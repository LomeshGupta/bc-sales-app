...
export async function POST() {
  const tenantId =
    process.env.BC_TENANT_ID || process.env.NEXT_PUBLIC_BC_TENANT_ID;
  const clientId =
    process.env.BC_CLIENT_ID || process.env.NEXT_PUBLIC_BC_CLIENT_ID;
  const clientSecret =
    process.env.BC_CLIENT_SECRET || process.env.NEXT_PUBLIC_BC_CLIENT_SECRET;
  const scope =
    process.env.BC_SCOPE ||
